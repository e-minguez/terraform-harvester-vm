...
- **No `storage_class_name` on VM disks** — the Harvester provider rejects `storage_class_name` on image-based disks ("can only be defined during image creation"). The `vm/` module does not expose a `storage_class_name` variable; control the storage class through `--image-storage-class` at image creation time instead.
